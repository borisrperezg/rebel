...
public class AMModel {
	...
	public Resource createModel(String projectName, String modelFileName) throws Exception {
		...
		if(ArchimateUtil.checkOrCreateFolder(projectName)) {
			...
			System.out.println("********************");
			System.out.println("AMModel.createModel ::: Model name = "+modelFileName);
			System.out.println("********************");
	...
	public void storeModel() throws Exception {
		...
		Date viewDate = project.getView().get(0).getDate();

		(new AMModelDerby()).findProjectAndInsertModel(projectName, modelName, 
				"archimate", uriAddress, (viewDate.getYear()+1900)+"-"+(viewDate.getMonth()+1)+"-"+viewDate.getDate());
