...
app.controller('RebelController', function($http, $scope, $sce, $location){
    ...
    $scope.getHtml2 = function(){
    	...
    	var pparam = getUrlParameter('p').replace('\%20', ' ');
    	var boiparam = getUrlParameter('b').replace('\%20', ' ');
    	
    	$scope.ruta = 'http://localhost:8080/rebelapi/rest/rebel/loadhtml/'+pparam+'&'+boiparam+'&'+factId;
...
//				console.log(response)
    ...
    this.listQAs = function(){
		...
		// ************************************************************
		// Se listas los tipos de causa que se pueden asociar al ATDi
		// ************************************************************
		this.listCauseTypes();
    };
    ...
    this.listCauseTypes = function(){
		...
		// ************************************************************
		// Se invoca el generador del html con los artefactos asociados
		// ************************************************************
		$scope.getHtml2();
    };
    ...
    // ============================================================
	// Se llama desde CreateBOI.html para listar todos los elementos
    // en la GUI
	// ============================================================
    this.showAndListModel = function () {
		...
		$http.get(ctrler.ruta).then(function(response) {
			// Carga los elementos encontrados en Archimate
			ctrler.elements = response.data;
			
	    });
		
		// Se listas los drivers definidos en Archimate
		this.loadDriverCombo();
    };
    
    // ============================================================
	// Se listas los drivers definidos en Archimate
	// ============================================================
    this.loadDriverCombo = function(){
    	...
    	this.ruta = 'http://localhost:8080/rebelapi/rest/rebel/listdrivers/'+projectName+"&"+modelName;
    	
		$http.get(ctrler.ruta).then(function(response) {
			// Carga los elementos encontrados en Archimate
			ctrler.drivers = response.data;
			
	    });
    	
    	// Se listan los goals de todos los drivers
    	this.loadGoalCombo();
    };
    ...
    this.loadGoalCombo = function(){
    	...
    	this.ruta = 'http://localhost:8080/rebelapi/rest/rebel/listgoals/'+projectName+"&"+modelName;
    	
		$http.get(ctrler.ruta).then(function(response) {
			// Carga los elementos encontrados en Archimate
			ctrler.goals = response.data;
...
//			console.log(response)
    ...
    this.loadModelBOI = function () {
    	...
    	// *****************************
    	// LOAD DRIVER AND GOAL
    	// *****************************
    	this.getDriverAndGoal();
    	...
    	// *****************************
		this.showTimeline();
		// Se quita de aca para colocarlo en el metodo
		// de WhyATD.html
		// $scope.getHtml2();		
		loadBOIModelIFrame();
    }
    
    this.getDriverAndGoal = function(){
    	var ctrler = this;
...
//    		console.log(response.data)
			...
			document.getElementById('goal').value = res[1];
	    });
    }
...
//				$window.location.reload();
    ...
    this.goTraining = function(){
    	...
    	var viewTemporal = document.getElementById("Views");
    	var viewTemporalName = viewTemporal.options[0].value;
    	
    	var n = viewTemporalName.indexOf("?")
    ...
    this.createboi = function () {
    	...
    	// Validacion del Driver ingresado
    	
    	var driverText = document.getElementById("driver").value;
    	
    	if (driverText == "") {
    		var driverCombo = document.getElementById("drivers");
        	driverText = driverCombo.options[driverCombo.selectedIndex].value;
    	}
    	
    	// Validacion del Goal ingresado
    	
    	var goalText = document.getElementById("goal").value;
    	
    	if (goalText == "") {
    		var goalCombo = document.getElementById("goals");
        	goalText = goalCombo.options[goalCombo.selectedIndex].value;
    	}
    	
    	
    	this.parametros = 'p='+document.getElementById('pname').value + 
    						'&bm=' + modelName + 
    						'&boi=' + document.getElementById('bname').value + 
    						'&dr=' + driverText + 
    						'&go=' + goalText;
