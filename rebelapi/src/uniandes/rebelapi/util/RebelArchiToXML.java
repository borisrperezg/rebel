 ...
 *
 */
public class RebelArchiToXML {

	public static void main(String[] args) throws Exception {
		RebelArchiToXML xml = new RebelArchiToXML();
		xml.xmlCreation("Bus-to-App_v3.rebel_archimate", "Demo");
	}
	...
	public boolean xmlCreation(String xmlFileName, String projectName) throws Exception  {
		...
		CoreModel cModel = new CoreModel();
		
		boolean resp = false;
		
		// **************************************************
		// APERTURA DEL MODELO REBEL
		// **************************************************
		
		// Open and read XML
		Resource res = cModel.loadResource("/Users/borisperezg/rebel/"+projectName+"/core/"+xmlFileName+".rebel_core");
		
		if(res!=null) {
			
			// **************************************************
			// RECORRIDO DE LAS VISTAS DE ARCHIMATE EN EL BOI
			// **************************************************
			Project project = (Project) res.getContents().get(0);
			List<ArchimateView> views = project.getView();
			if(views!=null) {
	        	for(ArchimateView view : views) {
	        		
	        		
	        		// **************************************************
	    			// CONFIGURACIÓN DE LAS CARPETAS DE DESTINO
	    			// **************************************************
	    			 ...
	    			 * 
	    			 * Validar que exista la carpeta y que el archivo no este previamente creado (esto ultimo queda pendiente)
	    			 */
	    			
	    			String xmlRunningFilePath = "/Users/borisperezg/apache-tomcat-7.0.100/webapps/rebelviz/archimatemodels/"+projectName+"/core/"+xmlFileName;
	    			if(createFolder(xmlRunningFilePath))
	    				xmlRunningFilePath += "/"+view.getName()+".xml";
	    			
	    			String xmlSourceFilePath = "/Users/borisperezg/eclipsews/rebelviz/WebContent/archimatemodels/"+projectName+"/core/"+xmlFileName;
	    			if(createFolder(xmlSourceFilePath))
	    				xmlSourceFilePath += "/"+view.getName()+".xml";
	        		
	    			// **************************************************
	    			// CONFIGURACIÓN DEL XML A GENERAR
	    			// **************************************************
	    	        ...
	    	        // **************************************************
	    			// CREACIÓN DEL CONTENIDO DEL XML
	    			// **************************************************
	    	        
		    	    // Procesando los elementos
	        		List<rebel_core.Element> elementNodes = view.getElement();
	        		if(elementNodes!=null) {
	        			for(rebel_core.Element elementNode : elementNodes) {
		    				...
		    				String isBoiElement = String.valueOf(elementNode.isBoiElement());
		    				
		    				root.appendChild(createMxCellElement(docGraph, id, name, 
		    						type, x, y, w, h, isGrouper, isBoiElement));
		    			}
		        	}
	        		
	        		// Procesando las relaciones
	        		List<rebel_core.Relation> relationNodes = view.getRelation();
	        		if(relationNodes!=null) {
	        			for(Relation relationNode : relationNodes) {
	        				
	        				rebel_core.Element elementSource = relationNode.getSource();
	        				rebel_core.Element elementTarget = relationNode.getTarget();
	...
	private Element createMxCellRelation(Document document, String id, String type, 
			rebel_core.Element elementSource, rebel_core.Element elementTarget) {
...
//        mxCell.setAttributeNode(attr);
	...
	private Element createMxGeometryRelation(Document document, rebel_core.Element elementSource, rebel_core.Element elementTarget) {
	 ...
	 */
	private int[] getSourceAndTargetPositions(rebel_core.Element elementSource, rebel_core.Element elementTarget) {
...
//		System.out.println("ubicaciones = "+ubicaciones);
	 ...
	 */
	private String getRelativePositionSourceVsTarget(rebel_core.Element elementSource, rebel_core.Element elementTarget) {
	...
	private Element createMxCellElement(Document document, String id, String value, 
			String type, String x, String y, String w, String h, String isGrouper, String isBoiElement) {
		Element mxCell = createMxCell(document, id, value, type, isGrouper, isBoiElement);
	 ...
	 * @return
	 */
	private Element createMxCell(Document document, String id, String value, String type, String isGrouper, String isBoiElement) {
		Element mxCell = document.createElement("mxCell");
...
//        System.out.println("Element Type = "+type);
        ...
        if(isBoiElement.equals("true"))
        	attr.setValue(attr.getValue()+"strokeWidth=2;");
