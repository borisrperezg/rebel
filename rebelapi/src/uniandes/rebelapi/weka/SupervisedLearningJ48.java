 ...
 * @see Evaluation
 */
public class SupervisedLearningJ48 {
	...
	public void execute() throws Exception {
		...
		m_Evaluation.crossValidateModel(m_Classifier, filtered, 4, m_Training.getRandomNumberGenerator(1));
	}
	...
	public static void main(String[] args) throws Exception {
		SupervisedLearningJ48 s = new SupervisedLearningJ48();
		double correct = s.training("multiclass_entrydataset_atdidentification", "csv");
	}

	public double training(String fileName, String extension) throws Exception {
...
//		String resp = "";
		...
		loader.setSource(new File(DATAFILESFOLDER+fileName+"."+extension));
		Instances data = loader.getDataSet();

		// save ARFF
		String arffFileName = fileName+".arff";
		...
		dataset = DATAFILESFOLDER+arffFileName;
		...
		// Used to transform input data, e.g. for preprocessing, transformation, feature
		// generation and so on.
		setFilter(filter, filterOptions.toArray(new String[filterOptions.size()]));
		// Machine learning algorithms
		setClassifier(classifier, classifierOptions.toArray(new String[classifierOptions.size()]));
		setTraining(dataset);
		execute();
